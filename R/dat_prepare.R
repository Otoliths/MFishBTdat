dat_prepare <- function(rebuild=FALSE, overwrite, pkgdir) {
  ...
  # a DESCRIPTION file with "Package: MFishBTdat" in the first line
  if (dir.exists(pkgdir)) {
    if (!file.exists(paste0(pkgdir, "/DESCRIPTION")))
      stop("No DESCRIPTION file in the package root directory.")
    if (readLines(paste0(pkgdir, "/DESCRIPTION"), n=1) != "Package: MFishBTdat")
      stop("DESCRIPTION file in the package root directory is not for the 'MFishBTdat' package.")
  } else {
